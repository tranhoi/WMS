...
namespace WebUIFinal.Pages.UnitPage
{
    public partial class UnitList
    {
        ...
        async Task AddNewItemAsync() => _navigation.NavigateTo($"/addUnit/Create New Unit");

        async Task EditItemAsync(int id) => _navigation.NavigateTo($"/addUnit/Edit Unit|{id}");
        async Task ViewItemAsync(int id)
        {

            _navigation.NavigateTo($"/addUnit/View Unit|{id}");
        }
        ...
        async Task DeleteItemAsync(Unit model)
        {
            try
            {
                var confirm = await _dialogService.Confirm($"Are you sure you want to delete Unit: {model.UnitName}?", "Delete Unit", new ConfirmOptions()
                {
                    OkButtonText = "Yes",
                    CancelButtonText = "No",
