...
namespace WebUIFinal.Pages
{
    public partial class LocationList
    {
        ...
        async Task DeleteItemAsync(Location model)
        {
            try
            {
                var confirm = await _dialogService.Confirm($"Are you sure you want to delete location: {model.LocationName}?", "Delete location", new ConfirmOptions()
                {
                    OkButtonText = "Yes",
                    CancelButtonText = "No",
        ...
        {
            _navigation.NavigateTo($"/addlocation/Edit Location|{id}");
        }

        async Task AddNewItemAsync()
        {
            _navigation.NavigateTo($"/addlocation/Create New Location");
        }

        async Task ViewItemAsync(string id)
        {

            _navigation.NavigateTo($"/addlocation/View Location|{id}");
        }
