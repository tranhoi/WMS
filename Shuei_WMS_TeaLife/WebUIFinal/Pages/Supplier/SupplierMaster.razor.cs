...
namespace WebUIFinal.Pages.Supplier
{
    public partial class SupplierMaster
    {
        ...
        async Task AddNewItemAsync() => _navigation.NavigateTo($"/detailsupplier/Create New Supplier");

        async Task EditItemAsync(int id) => _navigation.NavigateTo($"/detailsupplier/Edit Supplier|{id}");
        async Task ViewItemAsync(int id)
        {

            _navigation.NavigateTo($"/detailsupplier/Detail Supplier|{id}");
        }
        ...
        async Task DeleteItemAsync(SupplierEntity model)
        {
            try
            {
                var confirm = await _dialogService.Confirm($"Are you sure you want to delete this: {model.SupplierName}?", "Delete", new ConfirmOptions()
                {
                    OkButtonText = "Yes",
                    CancelButtonText = "No",
