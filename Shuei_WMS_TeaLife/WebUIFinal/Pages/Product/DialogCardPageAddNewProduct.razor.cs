...
namespace WebUIFinal.Pages.Product
{
    public partial class DialogCardPageAddNewProduct
    {
            ...
            if (ProductId.HasValue && ProductId > 0)
            {
                ...
                if (!product.Succeeded)
                {
                    _notificationService.Notify(new NotificationMessage()
                    {
                        ...
                        Summary = "Error",
                        Detail = product.Messages.FirstOrDefault(),
                        Duration = 1000
                    });
                ...
                _selectedCurrency = _currencyList.Where(x => x.CurrencyCode == model.CurrencyCode).FirstOrDefault();
                ...
                {
                    var imageInfo = model?.ProductImageName.Split("|");
                    fileName = model.ProductImageName = imageInfo[0];
        ...
        {
            var res = await _currencyServices.GetAllAsync();
            ...
            if (!res.Succeeded)
            {
                _notificationService.Notify(new NotificationMessage()
                {
                    Severity = NotificationSeverity.Error,
                    Summary = "Erorr",
                    Detail = $"Get currncy error: {res.Messages.FirstOrDefault()}",
                    Duration = 5000
                });
        ...
        async void Submit(ProductModel arg)
        {
            ...
            if (currencySelect == null)
            {
                ...
                {
                    Severity = NotificationSeverity.Error,
                    Summary = "Select currncy error",
                    Detail = "Currency do not exist.",
                    Duration = 5000
                });
            ...
            model.CurrencyCode = currencySelect.CurrencyCode;
            model.Currency = currencySelect.Country;
            ...
            if (Title.Contains(_localizer["Detail.Create"]))
            {
                ...
                payload.FileNameImage = $"{DateTime.Now.ToString("yyyyMMdd")}_{model.ProductCode}_{fileName}";
                payload.ProductImage = imageBase64String;
