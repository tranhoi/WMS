...
namespace WebUIFinal.Pages.Components
{
    public partial class DialogCardPageAddNewBin
    {
        ...
        async void Submit(Bin arg)
        {
            ...
            {
                var confirm = await _dialogService.Confirm($"Do you want to create a new Bin: {arg.BinCode}?", "Create bin", new ConfirmOptions()
                {
                    OkButtonText = "Yes",
                    CancelButtonText = "No",
            ...
            {
                var confirm = await _dialogService.Confirm($"Do you want to update bin: {arg.BinCode}?", "Update bin", new ConfirmOptions()
                {
                    OkButtonText = "Yes",
                    CancelButtonText = "No",
                    AutoFocusFirstElement = true,
