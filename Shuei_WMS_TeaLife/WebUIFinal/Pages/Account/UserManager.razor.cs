...
namespace WebUIFinal.Pages.Account
{
    public partial class UserManager
    {
        ...
        async Task DeleteItemAsync(UpdateDeleteRequestDTO model)
        {
            try
            {
                var confirm = await _dialogService.Confirm($"Are you sure you want to delete user: {model.Name}?", "Delete user", new ConfirmOptions()
                {
                    OkButtonText = "Yes",
                    CancelButtonText = "No",
        ...
        async Task ViewItemAsync(string id)
        {
            ...
            _navigation.NavigateTo($"/adduser/View User|{id}");
        }
        async Task EditItemAsync(string id)
        {

            _navigation.NavigateTo($"/adduser/Edit User|{id}");
        }

        async Task AddNewItemAsync()
        {
            _navigation.NavigateTo("/adduser/Create New Account");
